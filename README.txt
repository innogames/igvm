...
The Python library is called managevm. There are also two scripts you can use from
command line: buildvm and migratevm. Scipts are installed in /usr/local/bin
and Python library somewhere in PYTHONPATH. Just import the functions like this:
...
migratevm(vm_hostname, dsthv_hostname, newip=None, nopuppet=False, nolbdowntime=False, offline=False)
 ...
 Optional:
    ...
    offline        - boolean, allow offline migration, default is to attempt online migration
                     and fail if it is impossible due to hypervisor of network configuration

buildvm(vm_hostname, image=None, nopuppet=False, postboot=None)
  Mandatory:
    vm_hostname    - string, hostname of virtual machine
  Optional:
    image          - image to use as base for VM, if no image is given, one based on os Admintool
...
2a. Write the code.
2b. In order to test the code, run it the following way:
    python setup.py install --user && bin/(buildvm|migratevm) --params and commands
2c. Commit to your branch.
2d. Go to 2a until you are finished.
